...
export function ExamInterface({ questions, moduleTitle, companyId, moduleId, userId, userName, moduleType, isUnlocked }: ExamInterfaceProps) {
	...
	useEffect(() => {
		...
		const loadRetakeAccess = async () => {
			...
			try {
				...
				// If lookup fails, continue with regular user check
	...
	const handleNext = async () => {
		if (currentQuestionIndex < questions.length - 1) {
			...
			// Ensure current question's answer is included if it was selected
			let finalAnswers = [...answers]
			if (currentQuestion && selectedAlternativeId && hasAnswered) {
				// Check if current question's answer is already in the array
				const existingAnswer = finalAnswers.find(a => a.questionId === currentQuestion.id)
				if (!existingAnswer) {
					// Add current question's answer if not already included
					const selectedAlternative = currentQuestion.alternatives.find(a => a.id === selectedAlternativeId)
			...
			// Prepare answers for database - ensure we have an answer for every question
			const answerSubmissions: AnswerSubmission[] = questions.map(question => {
				...
				// If question wasn't answered, still include it with null values
				return {
					exerciseId: question.id,
					selectedAlternativeId: "", // Will need to handle this in saveExamResult
					isCorrect: false,
					timeSpentSeconds: 0
				}
			}).filter(answer => answer.selectedAlternativeId) // Only include answered questions
					...
					{questions.map((_, index) => {
						...
						return (
							<button
								...
								className={`h-2 rounded-full transition-all ${isCurrent ? "w-8 bg-primary" : hasAttempt ? "w-2 bg-green-500" : "w-2 bg-secondary"
									}`}
